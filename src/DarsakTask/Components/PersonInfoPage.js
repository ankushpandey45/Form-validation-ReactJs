...
const PersonPage = () => {
  ...
  useEffect(() => {
    ...
    if (employeeData) {
      setCmpnyName(JSON.parse(employeeData));
    }
  }, []);
    ...
    if (editTable !== null) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          setEditTable(null);
          setBtnName("Submit");
          localStorage.setItem("personInfoData", JSON.stringify(editedData));
      ...
      const updatedData = [
        ...tableData,
        { ...inputName, id: Math.floor(Math.random() * 999) },
      ];
      setTableData(updatedData);
      localStorage.setItem("personInfoData", JSON.stringify(updatedData));
      Swal.fire({
        position: "center",
        icon: "success",
